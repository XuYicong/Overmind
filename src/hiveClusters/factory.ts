...
export class InfestedFactory extends HiveCluster {
    ...
    spawnMoarOverlords(): void {
        // Infested Factory is attended by queens
    }
    ...
    private initFactoryStatus() {
		...
		// 		log.info(`${this.print}: starting production of ${components.map(([res, amount]) => `${amount * product.requested} of ${res}`).join(', ')} ` +
		...
		// 		if (_.all(components, ([component, amount]) => this.colony.assets[component] >= amount * (product.requested - produced))) {
		...
		// 		if (_.all(components,
		// 			([component, amount]) => this.factory.store[component] >= amount)) {
	...
	private statusTimeoutCheck(): void {
		...
		switch (this.memory.status) {
			case FactoryStatus.Idle:
				...
				break;
			case FactoryStatus.AcquiringComponents:
				timeout = ticksInStatus > FactoryStageTimeouts.AcquiringComponents;
				break;
			case FactoryStatus.LoadingFactory:
	...
	private registerRequests() {
		...
		// if (this.memory.status === FactoryStatus.Idle && Game.time - this.memory.statusTick > InfestedFactory.settings.dumpDelay) {
		...
		// 	const neededAmount = amount * (product.requested - produced) - this.factory.store[component];
		// 	if (neededAmount <= 0) continue;
		// 	const cappedAmount = minMax(neededAmount, 0, this.factory.store.getFreeCapacity() - InfestedFactory.settings.buffer);
		// 	// this.debug(() => `requesting input of ${neededAmount} of ${component}, capped at ${cappedAmount}`);
		// 	this.transportRequests.requestInput(this.factory, Priority.High, { resourceType: component, amount: cappedAmount });
		// }
		// const stored = Math.min(this.factory.store[product.commodityType], produced * product.size);
		// if (stored > 0) {
		// 	// this.debug(() => `requesting output of ${stored} of ${product.commodityType}`);
		// 	this.transportRequests.requestOutput(this.factory, Priority.Normal, { resourceType: product.commodityType, amount: stored });
		// }
	}
	...
	produce(commodity: CommodityConstant, amount: number = 1) {
		...
		// this.memory.activeProduction = { commodityType: commodity, requested: amount, size: COMMODITIES[commodity].amount };
    ...
    run(): void {
		...
		if (this.memory.suspendProductionUntil && Game.time > this.memory.suspendProductionUntil) {
		...
		// 		this.memory.produced = 0;
		// 	} else {
		// 		const sleepTime = getCacheExpiration(InfestedFactory.settings.sleepDelay, 10);
		...
		// if (this.memory.status === FactoryStatus.Producing && product && this.factory.cooldown === 0) {
		...
		// 			this.memory.stats.totalProduction[product.commodityType] = 0;
		// 		}
		// 		this.memory.stats.totalProduction[product.commodityType] += product.size;
		// 	} else {
